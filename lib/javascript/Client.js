...
function eapearson_GroupedParametersTest(url, auth, auth_cb, timeout, async_job_check_time_ms, service_version) {
    ...
    var _auth_cb = auth_cb;
     ...
     this.validate_checklist = function (checklist, _callback, _errorCallback) {
        if (typeof checklist === 'function')
            throw 'Argument checklist can not be a function';
        if (_callback && typeof _callback !== 'function')
            throw 'Argument _callback must be a function if defined';
        if (_errorCallback && typeof _errorCallback !== 'function')
            throw 'Argument _errorCallback must be a function if defined';
        if (typeof arguments === 'function' && arguments.length > 1+2)
            throw 'Too many arguments ('+arguments.length+' instead of '+(1+2)+')';
        return json_call_ajax(_url, "eapearson_GroupedParametersTest.validate_checklist",
            [checklist], 1, _callback, _errorCallback);
    };
